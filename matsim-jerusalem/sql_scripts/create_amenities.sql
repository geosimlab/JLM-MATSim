...
--conversion from bental to jtmt to matsim, adding taz to buildings, filter mixed residential, moch nodes and nkdg outside JLM metro
	...
	from
		poi_bldg
	left join bental_jtmt_code_conversion_long on
		poi_bldg.usg_group = bental_jtmt_code_conversion_long.usg_group
		and poi_bldg.usg_code = bental_jtmt_code_conversion_long.usg_code
	left join jtmt_matsim_code_conversion on
		jtmt_matsim_code_conversion.jtmt_code = bental_jtmt_code_conversion_long.value1
	left join taz600 on
		st_intersects(poi_bldg.geometry, taz600.geometry)
	where
...
--		creating opening hours
	...
	from
		trips
	left join jtmt_matsim_code_conversion on
		jtmt_matsim_code_conversion.jtmt_code = trips.destpurp
	group by
...
--		creating closing hours
	...
	from
		trips
	left join jtmt_matsim_code_conversion on
		jtmt_matsim_code_conversion.jtmt_code = trips.origpurp
	group by
		origtaz, MATSim_activity
	order by
		origtaz, MATSim_activity )
	select
		distinct poi_blds_coverted_taz.*, coalesce(taz_opening.opening_time_dest,-3 * 60) opening_time, coalesce(taz_closing.closing_time_orig, 30 * 60 - 3 * 60) closing_time
	from
		poi_blds_coverted_taz
	left join taz_opening on
		taz_opening.desttaz = poi_blds_coverted_taz.taz
		and taz_opening.MATSim_activity = poi_blds_coverted_taz.MATSim_activity
	left join taz_closing on
		taz_closing.origtaz = poi_blds_coverted_taz.taz
