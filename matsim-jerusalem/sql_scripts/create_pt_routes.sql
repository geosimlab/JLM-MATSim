...
lines.description,
links.length_met,
sum(line_path.stop) over(partition by line_path.line order by line_path.seq_number) cumsum_stop
from line_path
left join links 
...
summed as(
select line, cumsum_stop, sum (length_met) distance
from orig_pt_routes
group by line, cumsum_stop
order by line, cumsum_stop),
final_t as(
select orig_pt_routes.*, summed.distance 
from orig_pt_routes
left join summed
on orig_pt_routes.line = summed.line and orig_pt_routes.cumsum_stop = summed.cumsum_stop)
select line,seq_number, linkid, last_link,stop, description,distance,
case 
...
when transport_mode = 'b' then 15 end as stalling_time,
case 
