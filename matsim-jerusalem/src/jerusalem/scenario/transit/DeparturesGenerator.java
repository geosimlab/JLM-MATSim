...
public class DeparturesGenerator {
	...
	/**
	 * Creates departures for each given transit line of a given schedule according
	 * to the given headway, the start and end time of service and the slack time.
	 * 
	 */
	public void addDepartures(TransitSchedule schedule, List<Id<TransitLine>> lineIDs, double headway_sec,
			double startTime, double endTime, double pausenzeit) {
		...
		for (Id<TransitLine> transitLineId : lineIDs) {
			...
			for (TransitRoute transitRoute : this.transitSchedule.getTransitLines().get(transitLineId).getRoutes()
					.values()) {
			...
			if (routeId2transitRoute.size() > 2) {
				throw new RuntimeException(
						"A transit line consists of more than two transit routes. So far it is expected that a transit line consists of two identical"
								+ " transit routes (one for each direction). Can't garantee correct calculation of departure times. Aborting...");
			}

			if (routeId2transitRoute.size() < 2) {
				throw new RuntimeException(
						"A transit line consists of less than two transit routes. So far it is expected that a transit line consists of two identical"
								+ " transit routes (one for each direction). Can't garantee correct calculation of departure times. Aborting...");
			}
			...
			double routeTravelTime0 = routeId2transitRoute.get(routeIDs.get(0)).getStops().get(lastStop0)
					.getArrivalOffset();

			int lastStop1 = routeId2transitRoute.get(routeIDs.get(1)).getStops().size() - 1;
			double routeTravelTime1 = routeId2transitRoute.get(routeIDs.get(1)).getStops().get(lastStop1)
					.getArrivalOffset();

			if (routeTravelTime0 != routeTravelTime1) {
				throw new RuntimeException(
						"The transit routes have different travel times. So far it is expected that a transit line consists of two identical"
			...
			for (Id<TransitRoute> routeId : routeId2transitRoute.keySet()) {
				...
				if (routeNr == 1) {
					...
					log.info(routeId.toString() + ": first departure: "
							+ Time.writeTime(firstDepartureTime, Time.TIMEFORMAT_HHMMSS));
				} else if (routeNr == 0) {
